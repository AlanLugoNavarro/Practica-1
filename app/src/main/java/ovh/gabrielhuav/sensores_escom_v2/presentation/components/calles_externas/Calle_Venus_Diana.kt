...
import ovh.gabrielhuav.sensores_escom_v2.presentation.components.mapview.MapMatrixProvider.Companion.MAP_WIDTH

class Calle_Venus_Diana : AppCompatActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            mapView = MapView(
                context = this,
                mapResourceId = R.drawable.calle_venus_diana
            )
            ...
            mapView.post {
                // Configurar el mapa
                val normalizedMap = MapMatrixProvider.normalizeMapName(MapMatrixProvider.MAP_CALLE_VENUS_DIANA)
                mapView.setCurrentMap(normalizedMap, R.drawable.calle_venus_diana)
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            gameState.playerPosition = intent.getParcelableExtra("INITIAL_POSITION") ?: Pair(38, 37)
        } else {
    ...
    private fun initializeManagers() {
        bluetoothManager = BluetoothManager.getInstance(this, uiManager.tvBluetoothStatus).apply {
            setCallback(this@Calle_Venus_Diana)
        }
        ...
        val onlineServerManager = OnlineServerManager.getInstance(this).apply {
            setListener(this@Calle_Venus_Diana)
        }
...
//            }
    ...
    private fun startEstacionamientoActivity() {
        val intent = Intent(this, Estacionamiento::class.java).apply {
            ...
            putExtra("INITIAL_POSITION", Pair(37, 2)) // Posición inicial en el salón
    ...
    private var canChangeMap2 = false
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        canChangeMap = position.first >= 37 && position.first<= 39 && (position.second == 38 || position.second == 39)
        canChangeMap2 = position.first >= 36 && position.first<= MAP_WIDTH && position.second == 0

        if (canChangeMap) {
            runOnUiThread {
                Toast.makeText(this, "Presiona A para entrar al estacionamiento", Toast.LENGTH_SHORT).show()
            }
        }else if(canChangeMap2){
            runOnUiThread {
                Toast.makeText(this, "Presiona A para salir del estacionamiento", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun startBuildingActivity() {
        val intent = Intent(this, Calle_Venus_Diana::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
    ...
    private fun updatePlayerPosition(position: Pair<Int, Int>) {
        runOnUiThread {
            try {
                ...
                if (gameState.isConnected) {
                    serverConnectionManager.sendUpdateMessage(playerName, position, "calle venus-diana")
                }

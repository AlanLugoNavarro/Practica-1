...
class Estacionamiento : AppCompatActivity(),
    ...
    private fun initializeComponents(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            gameState.playerPosition = intent.getParcelableExtra("INITIAL_POSITION") ?: Pair(20, 38)
        } else {
    ...
    override fun onMapTransitionRequested(targetMap: String, initialPosition: Pair<Int, Int>) {
        when (targetMap) {
            ...
            MapMatrixProvider.MAP_CALLE_VENUS_DIANA -> {
                // Transición al salón 2009
                startCalleVenusDianaActivity()
            }
    ...
    // Método para iniciar la Activity de la escom
    private fun startMainActivity() {
        val intent = Intent(this, GameplayActivity::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
            putExtra("INITIAL_POSITION", Pair(35, 32)) // Posición inicial en el salón
    ...
    // Método para iniciar la Activity de la calle venus y diana
    private fun startCalleVenusDianaActivity() {
        val intent = Intent(this, Calle_Venus_Diana::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
            putExtra("INITIAL_POSITION", Pair(38, 20)) // Posición inicial en el salón
            putExtra("PREVIOUS_POSITION", gameState.playerPosition) // Guardar la posición actual para regresar
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
        }
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        ...
        canChangeMap = position.first >= 18 && position.first<= 21 && position.second == 39
        canChangeMap2 = position.first >= 36 && position.first<= MAP_WIDTH && position.second == 0

        if (canChangeMap) {
            runOnUiThread {
                Toast.makeText(this, "Presiona A para entrar al plantel", Toast.LENGTH_SHORT).show()
            }

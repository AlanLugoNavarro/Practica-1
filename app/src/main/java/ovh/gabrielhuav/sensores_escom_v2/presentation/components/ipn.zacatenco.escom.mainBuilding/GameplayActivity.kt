...
class GameplayActivity : AppCompatActivity(),
    ...
    private fun startEstacionamientoActivity() {
        val intent = Intent(this, Estacionamiento::class.java).apply {
            putExtra("PLAYER_NAME", playerName)
            putExtra("IS_SERVER", gameState.isServer)
            putExtra("INITIAL_POSITION", Pair(16, 1))
            putExtra("PREVIOUS_POSITION", gameState.playerPosition) // Guarda la posición actual
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
        }
        startActivity(intent)
    ...
    private fun checkPositionForMapChange(position: Pair<Int, Int>) {
        canChangeMap = position.second >= 10  && position.second<= 11 && position.first >= 15 && position.first<=16
        canChangeMap2 = position.first >= 36 && position.first <= 38 && position.second >= 31 && position.second <= 34
        if (canChangeMap) {
            runOnUiThread {
            ...
            runOnUiThread {
                Toast.makeText(this, "Presiona A para salir del plantel", Toast.LENGTH_SHORT).show()
            }
        }
    }

...
class MapMatrixProvider {
    companion object {
        ...
        const val MAP_ESTACIONAMIENTO = "estacionamiento"
        const val MAP_CALLE_VENUS_DIANA = "calle_venus_diana"
        ...
        fun normalizeMapName(mapName: String?): String {
            ...
            return when {
                ...
                lowerMap.contains("venus") || lowerMap.contains("diana") -> MAP_CALLE_VENUS_DIANA
        ...
        fun getMatrixForMap(mapId: String): Array<Array<Int>> {
            return when (mapId) {
                ...
                MAP_ESTACIONAMIENTO -> createEstacionamientoMapMatrix()
                MAP_CALLE_VENUS_DIANA -> createCalleVenusDianaMapMatrix()
        ...
        private fun createMainMapMatrix(): Array<Array<Int>> {
            ...
            for (i in 0 until MAP_HEIGHT) {
                for (j in 0 until MAP_WIDTH) {
                    ...
                    else if ((i >= 10  && i<= 11 && j >= 15 && j<=16) || (j >= 36 && j <= 38 && i >= 31 && i <= 34)) {
        ...
        private fun createEstacionamientoMapMatrix(): Array<Array<Int>> {
            val matrix = Array(MAP_HEIGHT) { Array(MAP_WIDTH) { PATH } }
            ...
            for (i in 0 until MAP_HEIGHT) {
                for (j in 0 until MAP_WIDTH) {
                    // Bordes exteriores
                    if (((i == MAP_HEIGHT-1 || i == MAP_HEIGHT-2) && ((j >= 0 && j<= 17) || (j >= 22 && j<= 28) || (j >= 33 && j<= MAP_WIDTH-1)))) {
                        matrix[i][j] = WALL
                    }else if(j <= 5 && i >= 2 && i<= MAP_HEIGHT-7){
                        matrix[i][j] = WALL
                    }
                    // Zonas interactivas (edificios, entradas)
                    else if ((j >= 18 && j<= 21 && i == 39) || (j >= 36 && j<= MAP_WIDTH && i == 0)) {
                        matrix[i][j] = INTERACTIVE // Entrada al edificio 2
                    }
                }
            }
        ...
        private fun createCalleVenusDianaMapMatrix(): Array<Array<Int>> {
            val matrix = Array(MAP_HEIGHT) { Array(MAP_WIDTH) { PATH } }
            ...
            for (i in 0 until MAP_HEIGHT) {
                for (j in 0 until MAP_WIDTH) {
                    // Bordes exteriores
                    if ((j == 36 && i >= 14) || (j <= 28 && j >= 8 && i <= 39 && i >= 9)){
                        matrix[i][j] = WALL
                    }else if((j == 0 && i <= 22 && i >= 14) || (j == 5 && i <= 22 && i >= 14) || (i == 14 && j >= 0 && j <= 5)){
                        matrix[i][j] = WALL
                    }
                    // Zonas interactivas (edificios, entradas)
                    else if ((j >= 37 && j<= 39 && (i == 38 || i == 39)) ) { //(j >= 36 && j<= MAP_WIDTH && i == 0)
                        matrix[i][j] = INTERACTIVE // Entrada al edificio 2
                    }
                }
            }
